...
| 26 | `TL_LearnPress_Course_Extension::save_course_audience_terms()` writes the `K-12` / `Professional Development` terms with `wp_set_object_terms( …, $append = true )` **plus** a `wp_remove_object_terms()` for the unticked ones. That is not redundant. Those two terms live in LearnPress's shared `course_category` taxonomy alongside subject categories (Math, Science, …), and `wp_set_object_terms()` defaults to `$append = false` — collapsing this into one replacing call silently wipes every other category off the course. These terms are **labels, not gates** — `get_available_courses()` offers every published course to every teacher and uses them only to pick the `Students` / `PD` badge. An untagged course is still offered, just unlabelled; do not reintroduce a filter on them. |
...
Creates, as one unit with compensating deletes on failure: a WP user (role `lp_teacher`, `user_login` = email) **and** a `tl_teacher` post (`lxp_teacher_admin_id`, `lxp_teacher_school_id`, `grades` JSON, `teacher_professional_development`, `settings_active`). Both halves are mandatory — `teacher-dashboard.php` hard-`die()`s for an `lp_teacher` user with no `tl_teacher` post.

### Grades and Professional Development

The grades block is always visible, and below it sits a single **Professional Development** checkbox. Signup requires **at least one grade *or* PD** — PD alone is enough, so an administrator registering faculty never has to claim a grade they do not teach. Only a wholly empty selection is refused, in the widget's JS and again in `Rest_Lxp_Teacher_Signup::signup()`.

PD is stored as its own `teacher_professional_development` meta (`'1'` / `'0'`) on the `tl_teacher` post, read via `lxp_get_teacher_pd_flag()`. It is deliberately **not** part of `grades`: that meta feeds `Rest_Lxp_Class::save_class_grades()`, which validates against `lxp_get_grade_options()` and seeds a class's `grade`/`lxp_class_grades` (gotcha #18), so a non-grade value in there would surface as a bogus grade on the class list.

> A short-lived earlier version asked a `teacher_register_type` radio (`k12` / `professional_development`) and used it to filter the course catalogue. Both are gone. Accounts created in that window still carry the meta; nothing reads it, and it is intentionally not migrated.

### Course audience labels

Two terms in **LearnPress's own `course_category` taxonomy** (slugs `k-12` and `professional-development`, `lxp_get_course_audience_terms()`), created idempotently by `tl_lxp_maybe_install_course_audience_terms()` on `init` priority 20 — not on activation, because LearnPress registers the taxonomy on `init` and the plugin is already active on running sites. Course authors tick them in the **Curriki Audience** meta box on the course edit screen.

They **gate nothing**. `Rest_Lxp_Class::get_available_courses()` returns every published course to every teacher; each term merely adds its `short` label (`Students` / `PD`) to that course's `audience` array, which the class modals render as a pill beside the title. A course tagged with neither is still offered, just unlabelled.
