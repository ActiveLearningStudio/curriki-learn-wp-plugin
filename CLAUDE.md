...
| 26 | `TL_LearnPress_Course_Extension::save_course_audience_terms()` writes the `K-12` / `Professional Development` terms with `wp_set_object_terms( …, $append = true )` **plus** a `wp_remove_object_terms()` for the unticked ones. That is not redundant. Those two terms live in LearnPress's shared `course_category` taxonomy alongside subject categories (Math, Science, …), and `wp_set_object_terms()` defaults to `$append = false` — collapsing this into one replacing call silently wipes every other category off the course. These terms are **labels, not gates** — `get_available_courses()` offers every published course to every teacher and uses them only to pick the `Student` / `PD` badge. An untagged course is still offered, just unlabelled; do not reintroduce a filter on them. |
